...
Для каждого intent зафиксировано:обязательные поля, правила уточнения, правила disambiguation.

3. Один уточняющий вопрос

Если отсутствует обязательное поле —возвращается:
...
8. Inbox и временные атрибуты

Inbox используется только для generic task без временных признаков:
- нет даты,
- нет времени,
- нет длительности,
- и intent не относится к timeblock.

Для `timeblock.create` fallback в Inbox запрещён.
Если у timeblock есть `duration`, но нет `start_at`, Runtime обязан вернуть уточнение:
`На какое время поставить блок?`

9. Clarifying precedence (temporal intents)

Для `timeblock.create` порядок уточнений фиксированный, ровно один вопрос за шаг:
- если нет `duration` -> `На сколько минут поставить блок?`
- иначе если нет `start_at` -> `На какое время поставить блок?`

Инвариант:
- temporal intents никогда не auto-fallback в Inbox;
- исполнение только после заполнения required temporal полей.
...
Ниже — канонические фразы для daily digest и его действий.
Требование: использовать формулировки дословно.
...
### Digest lines
- `Активные цели: {value}`
- `Просроченные цели: {value}`
- `Цели со сроком сегодня/завтра: {value}`
- `Цели под риском: {value}`
- `Задачи на сегодня: {value}`
- `Задачи на завтра: {value}`
- `Активные задачи: {value}`
...
### Goal item text
- `{index}. #{goal_id} {title} — просрочена на {days}д, переносов: {reschedule_count}`
- `{index}. #{goal_id} {title} — до срока {days}д, переносов: {reschedule_count}`
...
Запрещены эмоциональные и оценочные реплики в digest-сценарии.
